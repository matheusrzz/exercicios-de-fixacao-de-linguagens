# Aprendizado em C++ 🚀

Este repositório foi criado para armazenar todos os meus códigos, exercícios de fixação e projetos práticos desenvolvidos durante a minha jornada de estudos pelas linguagem de programação.

## 🎯 Objetivo
Armazenar e mostrar meu empenho nos estudos e desenvolvimento dos meus conhecimentos adqueridos em cursos e estudos a respeito das línguagens.

## 🗂️ Estrutura do Repositório
O projeto está organizado em pastas que seguem o cronograma de seções do curso:

* [ x] `secao-02-cpp/` — Exercícios voltados a linguagem C++.
* [ x] `secao-02-python/` — Exercícios voltados a linguagem Python.
* [ x] `secao-03-c-sharp/` — Exercícios voltados a linguagem C#.
* [ x] `secao-04-java/` — Exercícios voltados a linguagem JavaScript.

## 🛠️ Ferramentas

* **Ambiente de Desenvolvimento:** VS Code / IDE / Dev C++
